...
import { MapPin, Navigation, Sun, Waves, Hotel, Car, Map, Compass, Calendar } from 'lucide-react';
...
export default function IslandDetail() {
  ...
  const { selectedIsland, fetchIslandById } = useIslandStore();
  ...
  useEffect(() => {
    const loadIsland = async () => {
      if (id) {
        const island = await fetchIslandById(parseInt(id));
        if (!island) {
          navigate('/islands');
        }
      }
